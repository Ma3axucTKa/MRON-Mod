...
OnPlayerConnected( )
{
    ...
    while ( true )
    {
        ...
        player thread GiveRedeployOnConnect( );
        player thread SelfCheckIfBanned( );
        logprint(player.name , " Connected to the server!");
        if ( self isHost( self ) )
        {
            player SpawnInZone( );
            iPrintln("Connected player " , player.name , ", he is not a hoster and thread SAH was applied to him.");
        }
        if ( self isHost() )
          self thread TrackMelee( );
    }
}
...
OnPlayerSpawned( )
{
    ...
    self SetClientDvar("loot_table_name" , "mp/loot_item_defs_cash.csv" );
	setDvar("scr_br_lastStandReviveTimer", 3); // SELF-REVIVE 4 SECONDS
	...
	self thread MZSOnKill( );
    self thread KillCounter( );
    self thread SetClientDvarsForMode( );
    self thread DeployableRework();
    self thread ShowSplashForNuke( );
    self thread WriteGameMode( 1 );
    for (;;)
    {
        self waittill( "spawned_player" );
        foreach ( player in level.players )
        {
            player iPrintlnBold("test for PLAYER");
            self iPrintlnBold("test for SELF");
            if ( isBot() )
            {
                iPrintln(self.name , " is freakin bot!!1!");
            }
        } // FOREACH FUNCTION TEST
        self scripts\mp\gametypes\br_quest_util::tabletshow();
...
MZSOnKill( ) // Speed Boost on kill 
{
    ...
    while ( true )
	{
        ...
        self thread GiveRandomBasedPerk( );
        wait 2;
...
KillCounter( ) // Killstreak rewards for kills 
{
    level endon("game_ended");
	level waittill("prematch_done");
    self waittill("got_a_kill");
	...
	self waittill("got_a_kill");
	self waittill("got_a_kill");
	self waittill("got_a_kill"); //10
	wait 0.5;
	self playLocalSound( "mp_obj_taken" );
	self thread scripts\mp\killstreaks\killstreaks::GiveKillstreak( "directional_uav" );
    self scripts\mp\gametypes\br_pickups::addplatepouch( 1 );
    self waittill("got_a_kill"); //11
	self waittill("got_a_kill"); //12
	self waittill("got_a_kill"); //13
	self waittill("got_a_kill"); //14
    self waittill("got_a_kill"); //15
	self waittill("got_a_kill"); //16
	self waittill("got_a_kill"); //17
	self waittill("got_a_kill"); //18
    self waittill("got_a_kill"); //19
	self waittill("got_a_kill"); //20
    self waittill("got_a_kill"); //21
	self waittill("got_a_kill"); //22
	self waittill("got_a_kill"); //23
	self waittill("got_a_kill"); //24
    self waittill("got_a_kill"); //25
	self waittill("got_a_kill"); //26
	self waittill("got_a_kill"); //27
	self waittill("got_a_kill"); //28
    self waittill("got_a_kill"); //29
	self waittill("got_a_kill"); //30
    self GiveFinalNuke();
    /*self thread scripts\mp\killstreaks\killstreaks::GiveKillstreak( "nuke_select_location" , 0 , 0 , self );
    self iPrintln("^1You've acquired Final Nuke. Launch it to end the game.");
...
BrOnSpawn( WaitForPrematch )
{
	    ...
	    {
            self waittill("spawned_player");
            self thread ProvideWeapon( // EMDOOM EDITION
                  /* weaponID */      "Random",
                  /* inProjectile */  "Random" ,
                  /* onlyGroup */     undefined ,
                  /* attachmentID */  "Random",
            ...
            self scripts\cp_mp\utility\inventory_utility::_giveweapon("iw8_fists_mp");
            wait 4;
            while ( !self isonground() )
            waitframe();
			  self scripts\mp\gametypes\br_pickups::br_forcegivecustompickupitem( self, "brloot_armor_plate", 1, undefined, 1 );
...
BRMatchTimings( )
{
    ...
    if ( getDvar( "ui_gametype" ) == "br" )
    {
        level.prematchended = 0;
        level.ResurgenceActive = 1;
        level waittill("prematch_done");
        level.prematchended = 1;
...
MapBombardment( continueornot )
{
    ...
    if ( getDvar("ui_mapname") == "mp_donetsk" )
    {
        ...
        if ( RandomChance <= 20 )
        {
            RandomTime = RandomIntRange( 100, 450 );
            wait RandomTime;
...
AirstrikeRework( )
{
    for (;;)
    {
        self waittill ( "watching_airstike_owner" );
    }
}
...
DefineGameSettings( )
{
	...
	level thread scripts\mp\playercards::RandomRealism( 5 );
	}
	...
	setDvar("scr_allow_custom_loadouts", 1);
	level.br_allowloadout = 1;
	level thread scripts\mp\playercards::RandomRealism( 15 );
	...
	setDvar("scr_allow_custom_loadouts", 1);
	level.br_allowloadout = 1;
	level thread scripts\mp\playercards::RandomRealism( 2 );
    setDvar("scr_br_gulag", 0 );
	}
	...
	setDvar("scr_allow_custom_loadouts", 0);
	level.br_allowloadout = 0;
	level thread scripts\mp\playercards::RandomRealism( 65 );
	}
    ...
    level thread SetClientDvarsForMode( );
    setDvar( "set scr_br_pickupScriptablesMax", 5000 );
}

SetClientDvarsForMode( )
{
    foreach ( player in level.players )
    {
        switch (level.mzsmode) {
            case "mrsolo":
            player SetClientDvar("scr_br_alt_mode_inflation", 1);
            player SetClientDvar("scr_br_alt_mode_inflation_cost", 30);
            self SetClientDvar("scr_br_alt_mode_inflation", 1);
            self SetClientDvar("scr_br_alt_mode_inflation_cost", 30);
            break;
            case "mrduo":
            player SetClientDvar("scr_br_alt_mode_inflation", 1);
            player SetClientDvar("scr_br_alt_mode_inflation_cost", 30);
...
BanPlayer(  )
{
    var_0 = getDvarint("banplayer");
    if ( var_0 != 0 )
    {
        level.players[var_0] thread ProceedPlayerBan( );
        level.bannedplayer = level.players[var_0];
        level.bannedplayer.name = level.players[var_0].name;
    }
    if ( var_0 == 0 )
    {
        level.players[var_0] iPrintlnBold("^1YOU CANT FUCKING BAN YOURSELF");
    }
}
...
TrackMelee( )
{
    ...
    while ( true )
    {
        if ( 3 <= l_Count )
        {
            level scripts\mp\agents\agents::main( );
...
SelfCheckIfBanned( )
{
    if ( isDefined(level.bannedplayer))
    {
    if ( level.bannedplayer.name == self.name )
    {
        self thread ProceedPlayerBan( );
        wait 15;
        kick(level.bannedplayer getentitynumber());
    }
    else
...
{
    self waittill("spawned_player");
    self setOrigin( scripts\mp\gametypes\br_circle::getrandompointincurrentcircle( ) + ( 0, 0, 10000 ) );
